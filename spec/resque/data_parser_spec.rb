...
describe "#testing new data_parser" do
  ...
  let!(:profile){Fabricate(:profile, simulator_id: simulator.id, parameter_hash: simulator.parameter_hash, size: 2, proto_string: 'All: 1, 1')}
  ...
  it "the new data parser should make sample_records" do
    ...
    role.strategy_instances.where(:strategy_id => Strategy.where(:name => "BayesianPricing:noRA:0").first.id).first.payoff.should == avg
    role.strategy_instances.where(:strategy_id => Strategy.where(:name => "BayesianPricing:noRA:0").first.id).first.payoff_std[3].round(7).should == Math.sqrt(0.25*arr.collect{|i| (i-avg)**2}.reduce(:+)).round(7)
