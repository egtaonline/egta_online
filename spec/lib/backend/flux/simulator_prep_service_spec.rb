...
describe SimulatorPrepService do
  ...
  let(:simulator_prep_service){ SimulatorPrepService.new(transfer_connection) }
  ...
  describe '#prepare_simulator' do
    before do
      ...
      login_connection.should_receive(:exec!).with("cd #{Yetting.deploy_path} && unzip -uqq fake_name.zip -d fake_name-ver1 && chmod -R ug+rwx fake_name-ver1 && mv fake_name-ver1/#{Dir.entries('fake_name-ver1')[0]} fake_name-ver1/fake_name")
    end
