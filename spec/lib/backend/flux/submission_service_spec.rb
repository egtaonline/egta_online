...
describe SubmissionService do
  ...
  let(:submission_service){ SubmissionService.new(connection, "fake/remote/path") }
  ...
  context 'exercising submission command' do
    context 'successful queue' do
      ...
      before do
        connection.should_receive(:exec!).with("qsub -V -r n fake/remote/path/#{simulation.id}/wrapper").and_return(data)
    ...
    context 'random error' do
      ...
      before do
        connection.stub(:exec!).with("qsub -V -r n fake/remote/path/#{simulation.id}/wrapper").and_raise("Failure")
