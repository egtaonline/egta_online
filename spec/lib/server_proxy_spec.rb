...
describe ServerProxy do
  before(:each) do
    ...
    @simulation.update_attributes(:id => 41352)
  ...
  describe "#queue_pending_simulations" do
    ...
    it "should set up the folder hierarcy for the simulation" do
      should_exist("#{@server_proxy.location}/#{@simulator.name}-#{@simulator.version}/simulations/#{@simulation.id}")
      should_exist("#{@server_proxy.location}/#{@simulator.name}-#{@simulator.version}/simulations/#{@simulation.id}/simulation_spec.yaml")
      should_exist("#{@server_proxy.location}/#{@simulator.name}-#{@simulator.version}/simulations/#{@simulation.id}/features")
    end
  end
