  ...
  describe 'perform' do
    ...
    context 'multiple valid observations' do
      before do
        ...
        Simulation.stub(:where).with({ number: 3 }).and_return([simulation])
    ...
    context 'some failures' do
      before do
        ...
        Simulation.stub(:where).with({ number: 4 }).and_return([simulation])
        profile.stub_chain(:observations, :create!)
        profile.should_receive(:update_symmetry_group_payoffs)
      end
    ...
    context 'all failures' do
      before do
        ...
        Simulation.stub(:where).with({ number: 5 }).and_return([simulation])
      end
