...
describe DprGameScheduler do
  describe '#profile_space' do
    context 'single role' do
      let(:scheduler){ Fabricate(:dpr_game_scheduler, size: 3) }
      ...
      it { scheduler.profile_space.sort.should eql(['All: 3 A', 'All: 2 A, 1 B', 'All: 1 A, 2 B', 'All: 2 A, 1 C', 'All: 1 A, 2 C', 'All: 3 B', 'All: 2 B, 1 C', 'All: 1 B, 2 C', 'All: 3 C'].sort) }
    end

    context 'multiple roles' do
      let(:scheduler){ Fabricate(:dpr_game_scheduler, size: 7) }
      ...
      it { scheduler.profile_space.sort.should eql(['Role1: 3 A; Role2: 4 C', 'Role1: 3 A; Role2: 3 C, 1 D', 'Role1: 3 A; Role2: 2 C, 2 D', 'Role1: 3 A; Role2: 1 C, 3 D', 'Role1: 3 A; Role2: 4 D',
