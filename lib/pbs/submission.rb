module PBS
  ...
  class MASSubmission 
    ...
    attr_accessor :process_memory, :qos, :time_allotted_per_sample, :script, :samples, :players, :simulation_id, :nodes
    
    def initialize(pbs_proxy, samples, simulation_id, script)
      @process_memory = pbs_proxy.process_memory
      @qos = pbs_proxy.qos
      @samples = samples
      @time_allotted_per_sample = pbs_proxy.time_allotted_per_sample
      @script = script
    ...
    def wall_time
      @time_allotted_per_sample*samples
    end
