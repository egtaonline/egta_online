class NyxWrapper
  def self.create_wrapper(simulation)
    ...
    FileUtils.cp("#{Rails.root}/lib/wrapper-template", "#{Rails.root}/tmp/#{simulation.number}/wrapper")
    File.open("#{Rails.root}/tmp/#{simulation.number}/wrapper", "a") do |file|
      ...
      file.syswrite("\#PBS -o #{Yetting.deploy_path}/simulations/#{simulation.number}/out\n")
      file.syswrite("\#PBS -e #{Yetting.deploy_path}/simulations/#{simulation.number}/out\n")
      file.syswrite("\#PBS -M #{simulator.email}\n")
      file.syswrite("mkdir /tmp/${PBS_JOBID}; cd /tmp/${PBS_JOBID}; cp -r #{root_path}/* .; cp -r #{Yetting.deploy_path}/simulations/#{simulation.number} .\n")
      ...
      file.syswrite("cp -r /tmp/${PBS_JOBID}/#{simulation.number} #{Yetting.deploy_path}/simulations; /bin/rm -rf /tmp/${PBS_JOBID}")
    end
  end
end