...
Given /^that ([^"]*) belongs to that ([^"]*)$/ do |child, parent|
  ...
  child = child.gsub(/\W+/,'_').downcase.sub(/^_/, '')
  child_instance = instance_variable_get("@#{child}")
  if parent_instance.respond_to?("#{child.pluralize}")
    parent_instance.send(child.pluralize) << child_instance
  elsif parent_instance.respond_to?("#{child.split("_").last.pluralize}")
    parent_instance.send(child.split("_").last.pluralize) << child_instance
  end
  child_instance.save!
end
