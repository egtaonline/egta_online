class Game
  ...
  def as_json(options={})
    "{
       ...
       \"profiles\": #{Profile.collection.find(display_profiles).select(:sample_count => 1, 'symmetry_groups.role' => 1, 'symmetry_groups.strategy' => 1, 'symmetry_groups.count' => 1, 'symmetry_groups.players.payoff' => 1).to_a }
    }"
  end
