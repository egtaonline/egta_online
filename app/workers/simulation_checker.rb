class SimulationChecker
  ...
  def self.perform
    ...
    if Simulation.active.length > 0
      ...
      Account.all.each do |account|
        system("sudo rsync -e ssh #{account.username}@nyx-login.engin.umich.edu:/home/wellmangroup/many-agent-simulations/simulations/#{account.username} /home/deployment/current/db/")
  ...
  def self.check_for_errors(simulation)
    if File.open("#{Rails.root}/db/#{account.username}/#{simulation.number}/out").read == ""
      if File.exist?("#{Rails.root}/db/#{account.username}/#{simulation.number}/payoff_data")
